...
def upstair_terrain(stepwidth=0.33, stepheight=0.05, stepnum=40, slope=0.05, mode="terrain-fix", env_vecs=[]):
    add_height = 0
    env_info = []
    if mode == "stair-fix":
...
def subplane(basex=0, basez=0, endx=0):
    ...
    sh_colBox = p.createCollisionShape(p.GEOM_BOX, halfExtents=[boxHalfLength, boxHalfWidth, boxHalfHeight])
    id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=sh_colBox,
                           basePosition=[basex + boxHalfLength, 0, basez - boxHalfHeight],
                           baseOrientation=[0.0, 0.0, 0.0, 1])
    p.changeDynamics(id, -1, lateralFriction=FRICTION)
    return (endx, basez)
...
def substair(basex=0, basez=0, stepwidth=0.33, stepheight=0.05, stepnum=40, sign="up"):
    ...
    if sign == "up":
        for i in range(stepnum):
            id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=sh_colBox,
                                   basePosition=[basex + (i + 0.5) * stepwidth, 0, basez + (i + 0.5) * stepheight],
                                   baseOrientation=[0.0, 0.0, 0.0, 1])
            p.changeDynamics(id, -1, lateralFriction=FRICTION)
...
def subslope(basex=0, basez=0, slope=0.1, endx=0):
    ...
    # quaterion = p.getQuaternionFromEuler([0, -slope, 0])
    quaterion = [0, np.sin(-slope / 2.0), 0, np.cos(-slope / 2.0)]
    # print("constract:",quaterion,q_new)
    sh_colBox = p.createCollisionShape(p.GEOM_BOX, halfExtents=[boxHalfLength, boxHalfWidth, boxHalfHeight])
    id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=sh_colBox,
                           basePosition=[basex + boxHalfLength * np.cos(slope), 0,
                                         basez + boxHalfLength * np.sin(slope)], baseOrientation=quaterion)
    p.changeDynamics(id, -1, lateralFriction=FRICTION)
...
def downstair_terrain(stepwidth, stepheight, stepnum=40):
    ...
    for i in range(stepnum):
        id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=sh_colBox,
                               basePosition=[0.5 - boxHalfLength + i * steplen, 0, basez - (i + 1) * sth],
                               baseOrientation=[0.0, 0.0, 0.0, 1])
        p.changeDynamics(id, -1, lateralFriction=FRICTION)
...
def upslope_terrain(slope=0.05):
    ...
    if slope > 0:
        id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=sh_colBox,
                               basePosition=[0.15 + boxHalfLength * np.cos(slope), 0, boxHalfLength * np.sin(slope)],
                               baseOrientation=quaterion)
    else:
        id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=sh_colBox,
                               basePosition=[-1 + boxHalfLength * np.cos(slope), 0, -boxHalfLength * np.sin(slope)],
                               baseOrientation=quaterion)
    p.changeDynamics(id, -1, lateralFriction=FRICTION)
...
def gallop(stepwidth=0.3, stepnum=30):
    ...
    for i in range(stepnum):
        id = p.createMultiBody(baseMass=0, baseCollisionShapeIndex=cliff_Box,
                               basePosition=[current_x + stepwidth + cliff_len / 2.0, 0, 5],
                               baseOrientation=[0.0, 0.0, 0.0, 1])
        p.changeDynamics(id, -1, lateralFriction=FRICTION)
